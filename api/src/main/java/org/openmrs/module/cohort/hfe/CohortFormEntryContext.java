 ...
 * </p>
 */
public class CohortFormEntryContext {
	    ...
	    //private Stack<Map<ObsGroup, List<CohortObs>>> obsGroupStack = new Stack<Map<ObsGroup, List<CohortObs>>>();
	    //private ObsGroup activeObsGroup;
	    
	    private CohortM cohort;
	    //private Patient existingPatient;
	    private CohortEncounter existingEncounter;
	    private Map<Concept, List<CohortObs>> existingObs;
	   // private Map<Concept, List<Order>> existingOrders;
	    private Map<CohortObs, Set<CohortObs>> existingObsInGroups;

	    private Stack<Concept> currentObsGroupConcepts = new Stack<Concept>();
	    private List<CohortObs> currentObsGroupMembers;
	   ...
	   // private List<ObsGroupEntity> unmatchedObsGroupEntities = null;
	    ...
	    public CohortFormEntryContext(Mode mode2) {
	        this.mode = mode2;
	        setupExistingData((CohortEncounter) null);
	     ...
	     */
	   /* public void beginObsGroup(Concept conceptSet, CohortObs thisGroup, ObsGroup obsGroupSchemaObj) {
	        setObsGroup(thisGroup);
	        currentObsGroupConcepts.push(conceptSet);
	        activeObsGroup = obsGroupSchemaObj;
	        Map<ObsGroup, List<CohortObs>> map = new HashMap<ObsGroup, List<CohortObs>>();
	     ...
	     */
	   /* public void setObsGroup(CohortObs group) {
	        if (group == null) {
	            currentObsGroupMembers = null;
	        } else {
	            currentObsGroupMembers = new ArrayList<CohortObs>();
	            for (CohortObs o : group.getGroupMembers())
	    ...
	    /*public void endObsGroup() {
	        ...
	        if (!obsGroupStack.isEmpty()){
	            Map<ObsGroup, List<CohortObs>> map = obsGroupStack.peek();
	            for (Map.Entry<ObsGroup, List<CohortObs>> e : map.entrySet()){
	     ...
	     */
	    public CohortObs getObsFromCurrentGroup(Concept concept, Concept answerConcept) {
	        if (currentObsGroupMembers == null)
	            return null;
	        for (Iterator<CohortObs> iter = currentObsGroupMembers.iterator(); iter.hasNext(); ) {
	            CohortObs obs = iter.next();
	     ...
	     */
	    public void setupExistingData(CohortM cohort) {
	    	this.cohort=cohort;
	    }
	     ...
	     */
		public void setupExistingData(CohortEncounter encounter) {
			existingEncounter = encounter;
			existingObs = new HashMap<Concept, List<CohortObs>>();
			//existingOrders = new HashMap<Concept, List<Order>>();
			if (encounter != null) {
				for (CohortObs obs : encounter.getObsAtTopLevel(false)) {
					List<CohortObs> list = existingObs.get(obs.getConcept());
					if (list == null) {
						list = new LinkedList<CohortObs>();
	     ...
	     */     
	   /* public void setupExistingObsInGroups(Set<CohortObs> set){
	        for (CohortObs parent : set)       
	     ...
	     * @return
	     */
	    public CohortObs removeExistingObs(Concept question, Concept answer) {
	        List<CohortObs> list = existingObs.get(question);
	        if (list != null) {
	            for (Iterator<CohortObs> iter = list.iterator(); iter.hasNext(); ) {
	                CohortObs test = iter.next();
	    ...
	    * @return
	    */
	    public CohortObs removeExistingObs(List<Concept> questions, Concept answer) {
	        for (Concept question:questions){
	            CohortObs ret = removeExistingObs(question, answer);
	            if (ret != null)
	                return ret;
	        }
	        return null;
	     ...
	     */
	    public CohortObs removeExistingObs(Concept question, String numericAns) {
		 ...
		 */
		public List<CohortObs> removeExistingObs(Concept question) {
			List<CohortObs> list = existingObs.get(question);
			existingObs.remove(question);
			return list;
		}
	   ...
	   /**
	     ...
	     * @return
	     */
	    public CohortObs removeExistingObs(Concept question, Boolean answer) {
	        List<CohortObs> list = existingObs.get(question);
	        if (list != null) {
	            for (Iterator<CohortObs> iter = list.iterator(); iter.hasNext(); ) {
	                CohortObs test = iter.next();
	    ...
	    /*public CohortObs getNextUnmatchedObsGroup(String path) {
	        CohortObs ret = null;
	    	int unmatchedContenterCount = 0;
	        for (Map.Entry<CohortObs, Set<CohortObs>> e : existingObsInGroups.entrySet() ) {
	   ...
	   /*public Obs findBestMatchingObsGroup(List<ObsGroupComponent> questionsAndAnswers, String xmlObsGroupConcept, String path) {
	     ...
	     */
	    public CohortEncounter getExistingEncounter() {
	    ...
	    public Map<Concept, List<CohortObs>> getExistingObs() {
	        return existingObs;
	    }
