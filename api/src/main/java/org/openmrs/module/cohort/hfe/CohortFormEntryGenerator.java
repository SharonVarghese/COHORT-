...
public class CohortFormEntryGenerator implements CohortTagHandler,TagHandler {
	 public String applyMacros(String xml) throws Exception {
	    ...
	    *//**
	     ...
	     * @throws Exception
	     *//*
	    public String applyMacros(CohortFormEntrySession session, String xml) throws Exception {
	    ...
	    *//**
	     ...
	     * @throws Exception
	     *//*
	    public String applyTranslations(String xml, CohortFormEntryContext context) throws Exception {
	        Document doc = HtmlFormEntryUtil.stringToDocument(xml);
	        Node content = HtmlFormEntryUtil.findChild(doc, "htmlform");
	    ...
	    *//**
	    ...
	    public String applyUnmatchedTags(CohortFormEntrySession session, String xml) throws Exception {
	    ...
	    *//**
	     ...
	     * <p/>
	     *
	     * @param cohortFormEntrySession the current form entry session context
	     * @param xml     the xml string to process
	     * @return the xml string (which should now be html) after tag processing
	     * @throws Exception
	     *//*
	    public String applyTags(CohortFormEntrySession cohortFormEntrySession, String xml) throws Exception {
	        Document doc = HtmlFormEntryUtil.stringToDocument(xml);
	        Node content = HtmlFormEntryUtil.findChild(doc, "htmlform");
	        StringWriter out = new StringWriter();
	        applyTagsHelper(cohortFormEntrySession, new PrintWriter(out), null, content, null);
	        return out.toString();
	    }

	    private void applyTagsHelper(CohortFormEntrySession session, PrintWriter out, Node parent, Node node,
	                                 Map<String, CohortTagHandler> tagHandlerCache) {
	        if (tagHandlerCache == null)
	            tagHandlerCache = new HashMap<String, CohortTagHandler>();
	       // TagHandler handler = null;
	        CohortTagHandler handler1=null;
	        // Find the handler for this node
	        {
	            ...
	            if (name != null) {
	                if (tagHandlerCache.containsKey(name)) {
	                    ...
	                    handler1 =tagHandlerCache.get(name);
	                } else {
	                    handler1 = CohortHtmlFormUtil.getService().getHandlerByTagName(name);
	                    tagHandlerCache.put(name, handler1);
	                }
	            }
	        }

	        if (handler1 == null)
	            handler1 = this; // do default actions

	        try {
	            boolean handleContents = handler1.doStartTag(session, out, parent, node);

	            // Unless the handler told us to skip them, then iterate over any children
	            if (handleContents) {
	                if (handler1 != null && handler1 instanceof IteratingCohortTagHandler) {
	                    // recurse as many times as the tag wants
	                    IteratingCohortTagHandler iteratingHandler = (IteratingCohortTagHandler) handler1;
	            ...
	            handler1.doEndTag(session, out, parent, node);
	        } 
	        catch (BadFormDesignException e) {
	            out.print("<div class=\"error\">" + handler1 + " reported an error in the design of the form. Consult your administrator.<br/><pre>");
	    ...
	    *//**
	    ...
	    public String applyRoleRestrictions(String xml) throws BadFormDesignException {
	        ...
	        while (xml.contains("<restrictByRole")) {
	            ...
	            boolean result = CohortFormEntryGenerator.processRoleRestrictionLogic(includeStr);
	            CohortFormEntryGenerator.removeFirstTagOccurrence(sb, "restrictByRole", result);
	    ...
	    *//**
		...
		@Override
		public boolean doStartTag(CohortFormEntrySession session,
				PrintWriter out, Node parent, Node node)
				throws BadFormDesignException {
			if (node.getNodeType() == Node.TEXT_NODE) {
	            out.print(node.getNodeValue());
	        } else if (node.getNodeType() == Node.COMMENT_NODE) {
		...
		@Override
		public void doEndTag(CohortFormEntrySession session, PrintWriter out,
				Node parent, Node node) throws BadFormDesignException {
			 if (node.getNodeType() == Node.TEXT_NODE) {
		            // do nothing
		        } else if (node.getNodeType() == Node.COMMENT_NODE) {
		            // do nothing
