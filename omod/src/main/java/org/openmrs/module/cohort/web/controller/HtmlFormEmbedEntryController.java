...
//        }
    ...
    public ModelAndView handleSubmit(@ModelAttribute("command") CohortFormEntrySession session,
        ...
        try {
            List<FormSubmissionError> validationErrors = session.getController().validateSubmission(session.getContext(), request);
        ...
        try {
            ...
            CohortHtmlFormUtil.getService().applyActions(session);
            String successView = session.getReturnUrlWithParameters();
            if (successView == null)
                successView = request.getContextPath() + "/module/cohort/editcohort.form" + getQueryPrameters(request, session);
