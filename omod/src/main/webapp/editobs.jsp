...
<option value="${concept}"<c:if test="${concept ==cohortobs.concept}">selected</c:if>>${concept}</option>
</c:forEach>
</select>	
<c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>	
</spring:bind>	   
...
<option value="${location}"<c:if test="${location ==cohortobs.location}">selected</c:if>>${location}</option>
</c:forEach>
<c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
</select>
</tr>
...
<input type="text" name="obsDateTime" size="10" onFocus="showCalendar(this,60)"
						   id="obsDateTime" value="${status.value}" /><i style="font-weight: normal; font-size: 0.8em;">(<openmrs:message code="general.format"/>: <openmrs:datePattern /></i><br/><br/>
<c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
</spring:bind>
</tr>
