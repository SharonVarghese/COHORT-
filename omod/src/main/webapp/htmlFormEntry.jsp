            ...
            $j.post("<c:url value="/module/htmlformentry/deleteEncounter.form"/>",
                {     encounterId: "${command.encounter.encounterId}",
                 ...
                 function(data) {
                     ...
                     if (url == null || url == "") {
                         url = "${pageContext.request.contextPath}/editcohort.form?cid=${command.cohort.cohortId}";
                     }
...
<%-- yes you see, these are not nessasary!..
<div id="htmlFormEntryBanner">--%>
    ...
    <%--<div style="float: left" id="discardAndPrintDiv">--%>
        <%--<c:if test="${!inPopup}">--%>
            <%--<span id="discardLinkSpan"><a href="<c:choose><c:when test="${not empty command.returnUrlWithParameters}">${command.returnUrlWithParameters}</c:when><c:otherwise>${pageContext.request.contextPath}/patientDashboard.form?patientId=${command.cohort.cohortId}</c:otherwise></c:choose>" class="html-form-entry-discard-changes">${backMessage}</a></span> | --%>
        <%--</c:if>--%>
...
<c:if test="${command.context.mode != 'VIEW'}">
    <form id="htmlform" method="post" onSubmit="submitHtmlForm(); return false;">
        <input type="hidden" name="personId" value="${ command.cohort.cohortId }"/>
